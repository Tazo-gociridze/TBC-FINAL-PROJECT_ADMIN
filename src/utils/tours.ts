...
export const createTour = async (tourData: TourData): Promise<TourData | null> => {
    try {
        const { data, error } = await supabase
            .from('tours')
            .insert([tourData])
...
export const updateTour = async (tourData: TourData): Promise<TourData | null> => {
    try {
        const { data, error } = await supabase
            .from('tours')
            .update(tourData)
...
export const uploadImage = async (file: File): Promise<UploadImageResponse> => {
    try {
        ...
        if (!data?.path) {
            ...
            return { error: new Error("Image upload successful, but path is undefined.") };
        }
        ...
        if (error instanceof Error) {
           return { error };
             ...
             return { error: new Error("An unknown error occurred while uploading the image.") };
        }
    }
};